...
def ipv4_to_value(ipv4_addr):
    ...
    """
    ipv4_numeric = 0
    for byte in ipv4_addr.split("."):
        byte = int(byte)
        ipv4_numeric = (ipv4_numeric << 8) | byte
    return ipv4_numeric
...
def get_subnet_mask_value(slash):
    ...
    if len(parse) > 1:
        ...
        slash = parse[0]
    subnet = 0xFFFFFFFF << (32 - int(slash))
    subnet = subnet & 0xFFFFFFFF
    return subnet
...
def ips_same_subnet(ip1, ip2, slash):
    ...
    return ((ip1 & subnet) & 0xFFFFFFFF) == ((ip2 & subnet) & 0xFFFFFFFF)
...
def get_network(ip_value, netmask):
    ...
    return:   0x01020300
    """
    return (ip_value & netmask) & 0xFFFFFFFF
...
def find_router_for_ip(routers, ip):
    ...
    return: None
    """
    for router_ip, mask in routers.items():
        if ips_same_subnet(ip, router_ip, mask["netmask"]):
            return router_ip
    return None
...
#     print("Running ipv4_to_value test cases...")
#     assert 4294901760 == ipv4_to_value("255.255.0.0")
#     assert 16909060 == ipv4_to_value("1.2.3.4")

#     print("Running value_to_ipv4 test cases...")
#     assert "255.255.0.0" == value_to_ipv4(4294901760)
#     assert "1.2.3.4" == value_to_ipv4(16909060)

#     print("Running get_subnet_mask_value test cases...")
#     assert 4294901760 == get_subnet_mask_value("/16")
#     assert 4294966784 == get_subnet_mask_value("10.20.30.40/23")

#     print("Running ips_same_subnet test cases...")
#     assert ips_same_subnet("10.23.121.17", "10.23.121.225", "/23") is True
#     assert ips_same_subnet("10.23.230.22", "10.24.121.225", "/16") is False

#     print("Running get_network test cases...")
#     assert 16909056 == get_network(16909060, 4294967040)
